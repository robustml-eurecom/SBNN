...
from reactnet import reactnet, HardBinaryConv
...
### SBNN modifications ###
def count_positive_weights(model_student):
    my_sign = sign_function.apply
    positive_weights = 0
    for module in model_student.modules():
        if isinstance(module,HardBinaryConv):
            positive_weights_layer = torch.sum(my_sign(module.weights))#-torch.sum(torch.sign(module.weights)-1.0)
            positive_weights = positive_weights_layer if positive_weights==0 else positive_weights+positive_weights_layer
    return positive_weights

def count_total_weights(model_student):
    total_weights =  0
    for module in model_student.modules():
        if isinstance(module,HardBinaryConv):
            total_weights = torch.numel(module.weights) if total_weights==0 else  torch.numel(module.weights)+total_weights

    return total_weights
    
def evaluate_h(model_student,total_weights):
    positive_weights = count_positive_weights(model_student)
    positive_ratio = positive_weights / total_weights
    h = F.relu(positive_ratio - args.EC)
    return h,positive_weights,positive_ratio
### end SBNN modifications ###
...
def train(epoch, train_loader, model_student, model_teacher, criterion, optimizer, scheduler):
    ...
    with torch.no_grad():
        total_weights = count_total_weights(model_student)
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        ### SBNN modifications ###
        h, positive_weights, positive_ratio = evaluate_h(model_student, total_weights)
        with torch.no_grad():
            _lambda = 0 if h<=1e-5 else  args.gamma*loss / (h * (1-args.gamma))
        loss += _lambda*h
        ### end SBNN modifications ###
        ...
        if i%1000 == 0:
            progress.display(i)
            print('num +1s = %.0f / %.0f, ratio of connections = %.3f\n'%(positive_weights,total_weights, positive_ratio) )
        ### end SBNN modifications ###
