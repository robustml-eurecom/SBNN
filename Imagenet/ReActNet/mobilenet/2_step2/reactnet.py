...
class HardBinaryConv(nn.Module):
    def __init__(self, in_chn, out_chn, kernel_size=3, stride=1, padding=1):
        ...
        self.tau = nn.Parameter(torch.tensor(1.0).float().cuda())
        self.phi = nn.Parameter(torch.tensor(0.0).float().cuda())
    ...
    def forward(self, x):
        ...
        binary_weights_no_grad = torch.sign(real_weights)#scaling_factor * torch.sign(real_weights)
        cliped_weights = torch.clamp(real_weights, -1.0, 1.0)
        binary_weights = binary_weights_no_grad.detach() - cliped_weights.detach() + cliped_weights
        #print(binary_weights, flush=True)
        y = F.conv2d(x, self.tau*binary_weights+self.phi, stride=self.stride, padding=self.padding)
